...
class _TabularViewState extends State<TabularView> {
  ...
  void initState() {
    ...
    stateManager = PlutoGridStateManager(
        ...
        onChanged: (PlutoGridOnChangedEvent plutoGridOnChangedEvent) {
  ...
  //   return StreamBuilder<List<SaleModel>>(
  //     stream: isar.saleModels.where().watch(),
  //     // stream: isar.collection<SaleModel>().where().watch(),
  //     builder: (context, snapshot) {
  ...
  //         // var rows = testRows;
  //         var rows =
  //             snapshot.data!.toList().map((e) => e.toPlutoRow()).toList();
  //         // var rows2 =
  //         //     snapshot.data!.map((object) => object.toPlutoRow()).toList();
  //         return PlutoGrid(
  //           key: ValueKey(DateTime.now()),
  //           columns: tableViewColumns,
  //           rows: rows,
  ...
  //             // update the isar database when a cell is edited
  //             var row =
  //                 stateManager.getRowByIdx(plutoGridOnChangedEvent.rowIdx);
  //             var object = SaleModel()..fromPlutoRow(row!);
  //             isar.saleModels.put(object);
  //           },

  //           // onChanged: (PlutoOnChangedEvent event) {
  //           // Handle chan]ges here
  //           // },
  //         );
  //       } else {
  //         return const Center(child: CircularProgressIndicator());
